...
def menu(welcome: bool = True) -> dict:
    ...
    if choice == 0:
        ...
        return new_game
    if choice == 1: return load_safely()
    return {}
...
def manage(player: Character, outside: Inventory | None = None, in_battle: bool = False) -> None:
    ...
    while True:
        ...
        if choice_name == DONE: return
        elif choice_name in [ADD, DISCARD, EQUIP, UNEQUIP, HEAL]:
            choice_display: str = f"{choice_name.lower()}{' with' if choice_name == HEAL else ''}"

            item: str = input(f"\nWhat would you like to {choice_display}?\n(leave blank to cancel)\n>>> ")
            if item == "": continue
            count: int = get_positive_num(f"\nHow many would you like to {choice_display}?\n(leave blank to cancel)\n>>> ") if choice_name == ADD and item in outside.items and outside.items[item] > 1 or choice_name == DISCARD and item in player.inventory.items and player.inventory.items[item] > 1 else 1
            if count < 0: continue
            ...
            try:
                if choice_name == ADD:
                    ...
                    if item in WEAPON_BONUSES: player.equip_weapon(item)
                    else: player.equip_armor(item)
                elif choice_name == UNEQUIP:
                    if item in WEAPON_BONUSES: player.unequip_weapon()
                    else: player.unequip_armor(item)
                else: player.heal(item, in_battle)
            except (KeyError, ValueError): msg = "That's not the right kind of item\n"
            except (EquipmentError, InventoryError):
                msg = "You can't do that\n"
                if rollback:
                    if choice_name == ADD: outside.add(item, count)
    ...
    while True:
        ...
        while True:
            ...
            if choice == 1:
                ...
                continue
            if choice == 2: break
            offer_name: str = input("\nWhat type of item do you want to offer?\n(enter just the name, leave blank to cancel)\n>>> ").lower().strip()
...
A wizard walked up, and said to the ruler:
"Begone thou, as I shall now take thy throne, which rightfully belongs to thee!"

The ruler stood up, and said:
"Guards! Take this fool away!"

But, the guards did not heed his call to action.
They just stood there in a trance.

The wizard walked closer, and said:
"Thou are finished! Thy castle and throne now belongs to thee!"

With a swift yet thundering movement of his magic staff,
the ruler was pushed up and away from his throne by nothing but wind,
and started to feel the floor spin beneath him.

Spin, spin, and spin...
Then, just as suddenly as it started, it stops.

Everything becomes bright, and he falls over onto the grass.
"Grass...?" he utters to himself.

"The castle has ceramic tiling..."

To the north, he can see the forest.
To the east, he can see the ocean.
...
""" if not(trigger_encounter) else "")
        ...
        if raw.isdigit():
                ...
                if state["search_x"] == x and state["search_y"] == y: msg = "You can't search the same spot again\nGo somewhere else\n"
                    ...
                    if at_place:
                        if   at == VILLAGE: village(player)
                            ...
                            if state["finished boss"]: msg = "Do not fear, for your guards are free of the wizard's trance\n"
                            else:
                                battle_result = battle(player, [
                                    create_beast("Dragon"),
                            ...
                            if state["finished boss"]: msg = f"You have already defeated the evil wizard!\n"
                            else:
                                battle_result = battle(player, [
            ...
            elif destination == CASTLE and map_char_at(state["search_x"], state["search_y"]) != DEFENSES: msg = INVALID_DIR + "You must get through the evil wizard's defenses first\n"
...
With a swift yet thundering movement of his magic staff, he casted the wizard to a land far, far away.
